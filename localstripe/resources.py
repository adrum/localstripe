        ...
        for key, value in vars(self).items():
            if not key.startswith('_'):
                if isinstance(value, StripeObject):
                    ...
                    and len(value)
                    and isinstance(value[0], StripeObject)
                ):
...
class Charge(StripeObject):
        ...
        **kwargs,
        ...
        try:
            ...
            if source is not None:
                ...
                assert (
                    source.startswith('pm_')
                    or source.startswith('src_')
    ...
    def _initialize_charge(
        self, on_success=None, on_failure_now=None, on_failure_later=None
    ):
    ...
    @property
    def refunds(self):
        return Refund._api_list_all(
            '/v1/charges/' + self.id + '/refunds', charge=self.id
        )
    ...
    @classmethod
    def _api_list_all(
        cls, url, customer=None, created=None, limit=10, starting_after=None
    ):
        try:
            if customer is not None:
                assert type(customer) is str and customer.startswith('cus_')
            if created is not None:
                ...
                if type(created) is dict:
                    assert len(created.keys()) == 1 and list(created.keys())[
                        0
                    ] in ('gt', 'gte', 'lt', 'lte')
        ...
        li = super(Charge, cls)._api_list_all(
...
class Customer(StripeObject):
        ...
        **kwargs,
        ...
        try:
            ...
            if invoice_settings['default_payment_method'] is not None:
                ...
                assert invoice_settings['default_payment_method'].startswith(
                    'pm_'
                )
        ...
        self.tax_ids._list = [
            TaxId(customer=self.id, **data) for data in tax_id_data
        ]
    ...
    def _get_default_payment_method_or_source(self):
        if self.invoice_settings.get('default_payment_method'):
            ...
            return [
                s for s in self.sources._list if s.id == self.default_source
            ][0]
    ...
    def _api_update(cls, id, **data):
        if (
            'invoice_settings' in data
            and data['invoice_settings'].get('default_payment_method') == ''
        ):
    ...
    def _api_add_source(cls, id, source=None, **kwargs):
        ...
        if source_obj._attaching_is_declined():
            raise UserError(
                402, 'Your card was declined.', {'code': 'card_declined'}
            )
    ...
    def _api_retrieve_tax_id(cls, id, tax_id, **kwargs):
        ...
        for txi in obj.tax_ids._list:
            if txi.id == tax_id:
                return txi
        raise UserError(
            404, 'Customer ' + id + ' does not have a tax ID with ID ' + tax_id
        )
    ...
    @classmethod
    def _api_list_all(
        cls, url, email=None, limit=None, starting_after=None, **kwargs
    ):
        if kwargs:
            raise UserError(400, 'Unexpected ' + ', '.join(kwargs.keys()))
        ...
        li = super(Customer, cls)._api_list_all(
            url, limit=limit, starting_after=starting_after
        )
        if email is not None:
...
extra_apis.extend(
    (
        ...
        (
            'GET',
            '/v1/customers/{id}/subscriptions/{subscription_id}',
...
class Invoice(StripeObject):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if default_tax_rates is not None:
            default_tax_rates = [
                TaxRate._api_retrieve(tr) for tr in default_tax_rates
            ]
        ...
        pending_items = [
            ii
            for ii in InvoiceItem._api_list_all(
                None, customer=self.customer, limit=99
            )._list
        ...
        self.ending_balance = max(
            0, cus.account_balance - self.subtotal - self.tax
        )
    ...
    def total_tax_amounts(self):
        ...
        for il in self.lines._list:
            ...
            if il.tax_rates:
                ...
                tax_amounts = [
                    tr._tax_amount(il.amount) for tr in self.default_tax_rates
                ]
    ...
    def _get_next_invoice(
        ...
        subscription_proration_date = try_convert_to_int(
        ...
        pending_items = [
            ii
            for ii in InvoiceItem._api_list_all(
                None, customer=customer, limit=99
            )._list
            if ii.invoice is None
        ]
        if (
            ...
            and not subscription_items
            and not pending_items
        ):
            raise UserError(400, 'Bad request')

        simulation = (
            subscription_items is not None
            or subscription_prorate is not None
            or subscription_tax_percent is not None
            or subscription_default_tax_rates is not None
            or subscription_trial_end is not None
        )

        current_subscription = None
        li = [
            s
            for s in customer_obj.subscriptions._list
        ...
        if default_tax_rates is None:
            if subscription_default_tax_rates is not None:
                ...
                and current_subscription.default_tax_rates is not None
            ):
                default_tax_rates = [
        ...
        items = (
            subscription_items
            or (current_subscription and current_subscription.items._list)
            or []
        )
        for si in items:
            if subscription_items is not None:
        ...
        if not simulation and not current_subscription:
            ...
            if subscription is not None:
                ...
                for previous_invoice in previous._list:
                    ...
                    invoice_items.append(
                        InvoiceItem(
                            amount=-previous_invoice.subtotal,
    ...
    def _api_list_all(
        ...
        li = super(Invoice, cls)._api_list_all(
            url, limit=limit, starting_after=starting_after
        )
        if customer is not None:
            Customer._api_retrieve(customer)  # to return 404 if not existant
...
class InvoiceItem(StripeObject):
    ...
    @classmethod
    def _api_list_all(
        cls, url, customer=None, limit=None, starting_after=None
    ):
        try:
            if customer is not None:
                assert type(customer) is str and customer.startswith('cus_')
        ...
        li = super(InvoiceItem, cls)._api_list_all(
            url, limit=limit, starting_after=starting_after
        )
...
class PaymentIntent(StripeObject):
        ...
        **kwargs,
        ...
        try:
            ...
            if payment_method is not None:
                assert type(payment_method) is str
                assert (
                    payment_method.startswith('pm_')
                    or payment_method.startswith('src_')
                    or payment_method.startswith('card_')
                )
            if capture_method is not None:
                assert capture_method in (
                    'automatic',
                    'automatic_async',
                    'manual',
                )
    ...
    def _create_charge(self, on_failure_now):
        ...
        charge._initialize_charge(
            self._on_success, on_failure_now, self._report_async_failure
        )
    ...
    def status(self):
        ...
        if (
            self.latest_charge.status == 'succeeded'
            and not self.latest_charge.captured
        ):
    ...
    @classmethod
    def _api_authenticate(
        cls, id, client_secret=None, success=False, **kwargs
    ):
        if kwargs:
            raise UserError(400, 'Unexpected ' + ', '.join(kwargs.keys()))
    ...
    def _api_capture(cls, id, amount_to_capture=None, **kwargs):
        ...
        obj = cls._api_retrieve(id)
        obj.latest_charge._capture(
            amount=amount_to_capture, on_success=obj._on_success
        )
        return obj
...
extra_apis.extend(
    (
        ...
        (
            ...
            PaymentIntent._api_capture,
        ),
        (
            'POST',
            '/v1/payment_intents/{id}/_authenticate',
...
class PaymentMethod(StripeObject):
        ...
        **kwargs,
        ...
        if type == 'card':
            if not (2019 <= card['exp_year'] < 2100):
                raise UserError(
                    400, 'Bad request', {'code': 'invalid_expiry_year'}
                )
    ...
    @classmethod
    def _api_list_all(
        cls, url, customer=None, type=None, limit=None, starting_after=None
    ):
        try:
        ...
        li = super(PaymentMethod, cls)._api_list_all(
            url, limit=limit, starting_after=starting_after
        )
        li._list = [
            pm
            ...
            if billing_scheme == 'per_unit':
                ...
                for t in tiers:
                    assert (
                        ...
                        and (
                            t['up_to'] == 'inf'
                            or type(try_convert_to_int(t['up_to'])) is int
                        )
                    )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            if statement_descriptor is not None:
                assert (
                    type(statement_descriptor) is str
...
class Product(StripeObject):
    ...
    @classmethod
    def _api_list_all(
        cls, url, active=None, limit=None, starting_after=None, **kwargs
    ):
        if kwargs:
            raise UserError(400, 'Unexpected ' + ', '.join(kwargs.keys()))
        ...
        if query:
            ...
            for price in all_prices:
                if (
                    (
                        price.nickname
                        and query.lower() in price.nickname.lower()
                    )
                    or (
                        price.product
                        and query.lower() in price.product.lower()
                    )
                    or any(
                        query.lower() in str(v).lower()
                        for v in price.metadata.values()
                    )
                ):
...
class Refund(StripeObject):
        ...
        **kwargs,
        ...
        try:
            if charge is not None:
                ...
                assert type(
                    payment_intent
                ) is str and payment_intent.startswith('pi_')
            else:
    ...
    def _api_list_all(
        ...
        li = super(Refund, cls)._api_list_all(
            url, limit=limit, starting_after=starting_after
        )
        if charge is not None:
...
class SetupIntent(StripeObject):
    ...
    @classmethod
    def _api_cancel(
        cls, id, use_stripe_sdk=None, client_secret=None, **kwargs
    ):
        if kwargs:
            raise UserError(400, 'Unexpected ' + ', '.join(kwargs.keys()))
...
class Subscription(StripeObject):
    ...
    def _create_invoice(self):
        ...
        invoice = Invoice(
            ...
            default_tax_rates=[tr.id for tr in (self.default_tax_rates or [])],
        ...
        if invoice.status == 'paid':
            ...
            # payment fails:
            if (
                invoice.charge.status == 'pending'
                and PaymentMethod._api_retrieve(
    ...
    def _on_recurring_payment_failure(self, invoice):
        ...
        if (
            invoice.charge
            and PaymentMethod._api_retrieve(invoice.charge.payment_method).type
            == 'sepa_debit'
        ):
    ...
    def _update(
        ...
        if items is not None:
            ...
            if (
                self.items._list[0].plan.id != items[0]['plan']
                or self.items._list[0].quantity != items[0]['quantity']
            ):
                self.items = List(
                    '/v1/subscription_items?subscription=' + self.id
                )
        ...
        create_an_invoice = self.plan.billing_scheme == 'per_unit' and (
            self.plan.interval != old_plan.interval
            or self.plan.interval_count != old_plan.interval_count
        )
        if create_an_invoice:
            self._create_invoice()
    ...
    @classmethod
    def _api_list_all(
        cls, url, customer=None, status=None, limit=None, starting_after=None
    ):
        try:
            if customer is not None:
                assert type(customer) is str and customer.startswith('cus_')
        ...
        li = super(Subscription, cls)._api_list_all(
            url, limit=limit, starting_after=starting_after
        )
        if status is None:
            li._list = [
...
class TaxId(StripeObject):
    ...
    def __init__(
        self, country=None, customer=None, type=None, value=None, **kwargs
    ):
        if kwargs:
            raise UserError(400, 'Unexpected ' + ', '.join(kwargs.keys()))
...
class TaxCalculationLineItem(StripeObject):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        amount = try_convert_to_int(amount)
        quantity = try_convert_to_int(quantity) if quantity else 1

        try:
            assert _type(amount) is int
            assert _type(quantity) is int and quantity > 0
            if reference is not None:
                assert _type(reference) is str
            if tax_behavior is not None:
                assert tax_behavior in ('exclusive', 'inclusive')
        ...
        self.product = product
        self._tax_calculation_id = tax_calculation_id

        # Tax amounts will be calculated by TaxCalculation
        self.amount_tax = 0
        self.tax_breakdown = []

    def _calculate_tax(self, tax_rates):
        """Calculate tax for this line item based on provided rates."""
        total_tax = 0
        breakdown = []

        for rate_info in tax_rates:
            percentage = rate_info.get('percentage', 0)
            inclusive = rate_info.get('inclusive', False)
            jurisdiction = rate_info.get('jurisdiction', 'Unknown')
            tax_type = rate_info.get('tax_type', 'sales_tax')

            if self.tax_behavior == 'inclusive' or inclusive:
                # Tax is already included in amount
                tax_amount = int(
                    self.amount - (self.amount / (1 + percentage / 100))
                )
            else:
                # Tax is on top of amount
                decimal = Decimal(str(self.amount * percentage / 100.0))
                tax_amount = int(
                    decimal.quantize(Decimal('1.'), ROUND_HALF_UP)
                )
            ...
            breakdown.append(
                {
                    'amount': tax_amount,
                    'jurisdiction': {
                        'country': rate_info.get('country', 'US'),
                        'state': rate_info.get('state'),
                        'display_name': jurisdiction,
                        'level': rate_info.get('level', 'state'),
                    },
                    'sourcing': 'destination',
                    'tax_rate_details': {
                        'percentage_decimal': str(percentage),
                        'tax_type': tax_type,
                    },
...
class TaxCalculation(StripeObject):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            assert _type(line_items) is list and len(line_items) > 0
        except AssertionError:
            raise UserError(400, 'Bad request')
        ...
        # Validate customer_details if provided
        address = None
        address_source = None
        tax_ids = []
        ...
        if customer_details is not None:
            try:
                assert _type(customer_details) is dict
                if 'address' in customer_details:
                    address = customer_details['address']
                    assert _type(address) is dict
                    address_source = 'billing'
                if 'address_source' in customer_details:
                    address_source = customer_details['address_source']
                    assert address_source in ('billing', 'shipping')
                if 'tax_ids' in customer_details:
                    tax_ids = customer_details['tax_ids']
                    assert _type(tax_ids) is list
                if 'taxability_override' in customer_details:
                    taxability_override = customer_details[
                        'taxability_override'
                    ]
                    assert taxability_override in (
                        'none',
                        'customer_exempt',
                        'reverse_charge',
                    )
        ...
        self.customer = customer
        self.tax_date = tax_date or self.created

        # Set expiration to 90 days from now (Stripe's default)
        self.expires_at = self.created + (90 * 24 * 60 * 60)
        ...
        # Store shipping cost if provided
        self.shipping_cost = None
        if shipping_cost is not None:
            amt = shipping_cost.get('amount', 0)
            shipping_amount = try_convert_to_int(amt)
            self.shipping_cost = {
                ...
                'tax_breakdown': [],
                'tax_code': shipping_cost.get('tax_code', 'txcd_92010001'),
            }

        # Store ship_from_details if provided
        self.ship_from_details = None
        if ship_from_details is not None:
            self.ship_from_details = {
                'address': ship_from_details.get('address', {}),
            }

        # Create line item objects
        self._line_items_list = []
        for item in validated_line_items:
            line_item = TaxCalculationLineItem(
                amount=item.get('amount'),
                quantity=item.get('quantity', 1),
                reference=item.get('reference'),
                tax_behavior=item.get('tax_behavior'),
                tax_code=item.get('tax_code'),
                product=item.get('product'),
                tax_calculation_id=self.id,
            )
            self._line_items_list.append(line_item)

        # Calculate taxes
        self._calculate_taxes()

    def _calculate_taxes(self):
        """Calculate taxes for all line items based on customer location."""
        # Determine tax rates based on customer address
        ...
        for line_item in self._line_items_list:
            tax_amount = line_item._calculate_tax(tax_rates)

            if line_item.tax_behavior == 'inclusive':
                total_inclusive += tax_amount
            else:
                total_exclusive += tax_amount

            total_amount += line_item.amount

            # Aggregate tax breakdown
            for tb in line_item.tax_breakdown:
                key = (
                    tb['jurisdiction'].get('country', ''),
                    tb['jurisdiction'].get('state', ''),
                    tb['tax_rate_details']['tax_type'],
                )
                if key not in tax_breakdown_map:
                    tax_breakdown_map[key] = {
                        'amount': 0,
                        'inclusive': line_item.tax_behavior == 'inclusive',
                        'tax_rate_details': tb['tax_rate_details'],
                        'taxability_reason': tb['taxability_reason'],
                        'taxable_amount': 0,
                    }
                tax_breakdown_map[key]['amount'] += tb['amount']
                taxable = tb['taxable_amount']
                tax_breakdown_map[key]['taxable_amount'] += taxable
        ...
        if self.shipping_cost:
            ...
            for rate_info in tax_rates:
                percentage = rate_info.get('percentage', 0)
                if self.shipping_cost['tax_behavior'] == 'exclusive':
                    decimal = Decimal(
                        str(self.shipping_cost['amount'] * percentage / 100.0)
                    )
                    tax = int(decimal.quantize(Decimal('1.'), ROUND_HALF_UP))
                    shipping_tax += tax

                    self.shipping_cost['tax_breakdown'].append(
                        {
                            'amount': tax,
                            'jurisdiction': {
                                'country': rate_info.get('country', 'US'),
                                'state': rate_info.get('state'),
                                'display_name': rate_info.get(
                            ...
                            'tax_rate_details': {
                                'percentage_decimal': str(percentage),
                                'tax_type': rate_info.get(
                                    'tax_type', 'sales_tax'
                                ),
                            },
                            'taxability_reason': 'standard_rated',
                            'taxable_amount': self.shipping_cost['amount'],
                        }
                    )

            self.shipping_cost['amount_tax'] = shipping_tax
            total_exclusive += shipping_tax
            total_amount += self.shipping_cost['amount']

        self.tax_amount_exclusive = total_exclusive
        self.tax_amount_inclusive = total_inclusive
        self.amount_total = total_amount + total_exclusive

        # Build overall tax breakdown
        self.tax_breakdown = list(tax_breakdown_map.values())

    def _get_tax_rates(self):
        """Get applicable tax rates based on customer location."""
        # Check if customer is exempt
        if (
            self.customer_details.get('taxability_override')
            == 'customer_exempt'
        ):
            return []

        # Get address for tax calculation
        address = self.customer_details.get('address') or {}
        country = address.get('country', 'US')
        state = address.get('state')

        # Default tax rates by country/state (simplified mock implementation)
        ...
        if country == 'US':
            ...
            if state and state in state_rates:
                tax_rates.append(
                    {
                        'percentage': state_rates[state],
                        'inclusive': False,
                        'jurisdiction': state,
                        'country': 'US',
                        'state': state,
                ...
                # Default 5% for unknown US states
                tax_rates.append(
                    {
                        'percentage': 5.0,
                        'inclusive': False,
            ...
            # Provincial taxes
            provincial_rates = {
                'ON': 13.0,  # HST (includes GST)
                'BC': 7.0,  # PST
                'QC': 9.975,  # QST
                'AB': 0.0,  # No PST
            }
            if state and state in provincial_rates:
                if state == 'ON':
                    # HST replaces GST
                    tax_rates = [
                        {
                ...
                elif provincial_rates[state] > 0:
                    tax_rates.append(
                        {
                            'percentage': provincial_rates[state],
                            'inclusive': False,
                            'jurisdiction': state,
                            'country': 'CA',
                            'state': state,
    ...
    @property
    def line_items(self):
        """Return line items as a List object."""
        li = List('/v1/tax/calculations/' + self.id + '/line_items')
        li._list = self._line_items_list
        return li
    ...
    def _api_retrieve(cls, id):
        ...
        # Check if calculation has expired
        if obj.expires_at < int(time.time()):
            raise UserError(404, 'Tax calculation has expired')
...
extra_apis.extend(
    (
        ('POST', '/v1/tax/calculations', TaxCalculation._api_create),
        ('GET', '/v1/tax/calculations/{id}', TaxCalculation._api_retrieve),
        (
            'GET',
            '/v1/tax/calculations/{id}/line_items',
            TaxCalculation._api_list_line_items,
        ),
    )
)
